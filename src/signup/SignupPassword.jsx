...
//     if (!userId || !token) {
//       setPasswordError(
//         "Missing user identification. Please try signing up again."
//       );
//       return;
//     }
...
//     /(?=.*\d)(?=.*[!@#$%^&*])/.test(password), // number + symbol
//   ];

//   const filledBars = checks.filter(Boolean).length;
...
//       <path d="M12.6666 7.33333H3.33325..." stroke="#9CA3AF" />
//     </svg>
//   );
...
//           fill="none"
//         >
//           <path d="M9.16667 3.75..." fill="#6B7280" />
//         </svg>
//       ) : (
//         /* YOUR ORIGINAL CLOSED EYE SVG */
//         <svg
//           xmlns="http://www.w3.org/2000/svg"
//           width="20"
//           height="20"
//           fill="none"
//         >
//           <path d="M1.66732 4.39159..." fill="#6B7280" />
//         </svg>
//       )}
//     </div>
//   );
...
//       >
//         <AuthHeading>You’re Almost In!</AuthHeading>
//         <AuthText>Set a secure password to continue</AuthText>
...
//                     height: 6,
//                     borderRadius: 6,
//                     backgroundColor: i < filledBars ? COLOR_GREEN : COLOR_GREY,
...
//         {confirmError && (
//           <p style={{ color: "#DC2626", fontSize: 12 }}>{confirmError}</p>
//         )}
...
const SignupPassword = ({
  ...
  // PASSWORD STRENGTH LOGIC
  // =================================================================
  // Matches: 8+ chars, 1 upper, 1 lower, 1 digit, 1 special
  const PASSWORD_REGEX =
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*]).{8,}$/;
  ...
  const getSegmentColor = (index, currentStrength) => {
    ...
    // (Only if we haven't reached full strength yet)
  ...
  // =================================================================
  const handleSetPassword = async () => {
    // 1. Reset all errors FIRST to clear UI
    setPasswordError("");
    setConfirmError("");
    setSuccessMessage("");
    ...
    console.log(isFormatValid, isMatchValid);
